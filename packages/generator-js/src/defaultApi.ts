import { BookElementProps, BookItem, LevelBookElements, isTemplateParams, templateToList } from "@bookbox/core";
...
const getElement: (name: string) => (props: BookElementProps) => GetSchema =
  ...
  (props) =>
  (...children) => {
    const list = isTemplateParams(children)
...
const getRootProxy =
  getElementProxy<Omit<BookApi, keyof UtilApi | LevelApiName>>();

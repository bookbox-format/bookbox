import { BookElements, MediaElements } from './elements';
import { BookBuilder, BookElementSchema, BookHeader, BookSchema, BookStore, BuildTokens } from './model';
...
function getObjectsByHeader({ condition }: { condition: (item: BookElementSchema) => boolean }) {
...
const getContents = <T>(builder: BookBuilder<T>, store: BookStore<T>, build: BuildTokens<T>) => {
  return getGroup({
